...
public class BankTransSourceController extends BaseController<BankTransSource> implements Serializable{
private static final long serialVersionUID = 1L;
private static final Logger logger = Logger.getLogger(BankTransSourceController.class);
...
setMessage("BankTransSource with id: " + getEntityList().get(row).getId()
...
setEntity(this.bankTransSourceService.getEntityById(getEntityList().get(row).getId()));
return "bankTransSource?faces-redirect=true&entityId=" + getEntityList().get(row).getId();
}
...
public IBankTransSourceService getPersonService() {
